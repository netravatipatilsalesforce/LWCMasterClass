...
export default class PasswordStrengthIndicator extends LightningElement 
{
    ...
    @track rules = [
        {
            label : 'Minimum 8 Charecters',
            test : (pwd) =>pwd.length >= 8
        },
         {
            label : 'Atleast 1 Uppercase Character',
            test : (pwd) => /[A-Z]/.test(pwd)
        },
         {
            label : 'Atleast 1 Lowercase Character',
            test : (pwd) => /[a-z]/.test(pwd)
        },
         {
    ...
    //Initially inputType = Password -> label = 'Show'
    //If we change it to inputType = 'text' -> label = 'Hide
    togglePassword()
    {
